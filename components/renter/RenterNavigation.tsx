...
interface RenterNavigationProps {
  userFullName?: string;
  userAvatar?: string;
  isVerified?: boolean;
}
...
export default function RenterNavigation({
  ...
  const navLinks = [
    { href: '/renter/browse', label: 'Browse' },
    { href: '/renter/bookings', label: 'Trips' },
  ];
    ...
    <>
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16 sm:h-20">
            {/* Logo */}
            <Link href="/renter/dashboard" className="flex items-center gap-2 flex-shrink-0">
              <Image
          ...
          <>
            ...
            <div className="md:hidden fixed top-0 right-0 bottom-0 w-80 max-w-[85vw] bg-white shadow-2xl z-50 overflow-y-auto">
              ...
              <div className="p-6 pt-20 sm:pt-24">
                ...
                <nav className="mt-6 space-y-1">
                  <Link
                    ...
                    className={`flex items-center gap-3 px-4 py-3 rounded-lg text-base font-medium transition-colors ${
                      isActive('/renter/browse')
                        ? 'bg-gray-100 text-gray-900'
                        : 'text-gray-700 hover:bg-gray-50'
                    }`}
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    <MagnifyingGlass size={24} weight={isActive('/renter/browse') ? 'bold' : 'regular'} />
                    Browse
                  </Link>
                  ...
                  <Link
                    ...
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    <Calendar size={24} weight={isActive('/renter/bookings') ? 'bold' : 'regular'} />
                    Trips
                  </Link>
                  ...
                  <Link
                    href="/renter/profile"
                    className={`flex items-center gap-3 px-4 py-3 rounded-lg text-base font-medium transition-colors ${
                      isActive('/renter/profile')
                        ? 'bg-gray-100 text-gray-900'
                        : 'text-gray-700 hover:bg-gray-50'
                    }`}
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    <User size={24} weight={isActive('/renter/profile') ? 'bold' : 'regular'} />
                    Profile
                  </Link>
                  ...
                  <Link
                    ...
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    <House size={24} weight={pathname === '/renter/dashboard' ? 'bold' : 'regular'} />
                    Dashboard
                  </Link>
                </nav>
                ...
                {/* Verification CTA */}
                {!isVerified && (
                  <div className="mt-6 p-4 bg-gradient-to-r from-primary-50 to-orange-50 rounded-xl border border-primary-200">
                    <p className="text-sm font-semibold text-gray-900 mb-2">Complete your profile</p>
                    <p className="text-xs text-gray-600 mb-3">
                      Get verified to start booking vehicles
                    </p>
                    <Link
                      href="/renter/profile"
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-40 safe-area-bottom">
        <nav className="grid grid-cols-4 gap-1 px-2 py-2">
          ...
          >
            <MagnifyingGlass size={24} weight={isActive('/renter/browse') ? 'fill' : 'regular'} className="mb-1" />
            <span className={`text-xs ${isActive('/renter/browse') ? 'font-bold' : 'font-medium'}`}>
              Browse
            </span>
          </Link>
          ...
          >
            <Calendar size={24} weight={isActive('/renter/bookings') ? 'fill' : 'regular'} className="mb-1" />
            <span className={`text-xs ${isActive('/renter/bookings') ? 'font-bold' : 'font-medium'}`}>
              Trips
            </span>
          </Link>
          ...
          >
            <User size={24} weight={isActive('/renter/profile') ? 'fill' : 'regular'} className="mb-1" />
            <span className={`text-xs ${isActive('/renter/profile') ? 'font-bold' : 'font-medium'}`}>
              Profile
            </span>
            {!isVerified && (
          ...
          >
            <House size={24} weight={pathname === '/renter/dashboard' ? 'fill' : 'regular'} className="mb-1" />
            <span className={`text-xs ${pathname === '/renter/dashboard' ? 'font-bold' : 'font-medium'}`}>
              Home
            </span>
          </Link>
        </nav>
